...
namespace YangOne.Data.Crud
{
    ...
    public static class CRUDHelper
    {
        ...
        public static Task<int> UpdateStatusByColumnName<T>(this IDbConnection connection, object id,object status,string columnName, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            IDatabaseFactory factory = connection.GetFactory();
            var sql = factory.QueryBuilder.UpdateStatusByColumnName<T>(id,status,columnName);
            factory.DbLogger.Log(LogType.Trace, () => $"starting--{sql.QuerySql} --datetime-->{DateTime.Now:yyyy_MM_d_hh_mm}");
            return connection.ExecuteAsync(sql.QuerySql, sql.Parameters, transaction, commandTimeout);
        }
        public static Task<int> UpdateStatus<T>(this IDbConnection connection, object id, object status, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            IDatabaseFactory factory = connection.GetFactory();
        ...
        public static Task<int> UpdateAsDeleted<T>(this IDbConnection connection, string condition, object parameters = null,
            IDbTransaction transaction = null, int? commandTimeout = null)
        {
            IDatabaseFactory factory = connection.GetFactory();
            var sql = factory.QueryBuilder.UpdateAsDeleted<T>(condition, parameters);
            factory.DbLogger.Log(LogType.Trace, () => $"starting--{sql.QuerySql} --datetime-->{DateTime.Now:yyyy_MM_d_hh_mm}");
            return connection.ExecuteAsync(sql.QuerySql, sql.Parameters, transaction, commandTimeout);
        }
