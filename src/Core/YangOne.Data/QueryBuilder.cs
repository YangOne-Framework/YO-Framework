...
using EditableAttribute = YangOne.Data.Crud.Attribute.EditableAttribute;
...
namespace YangOne.Data
{
    ...
    public abstract class QueryBuilder
    {
        ...
        public QueryRequest UpdateStatusByColumnName<T>(object id, object status,string columnName)
        {
            var currenttype = typeof(T);
            var idProps = GetIdProperties(currenttype).ToList();
            ...
            var sb = new StringBuilder();
            sb.AppendFormat("update {0}", name);
            sb.AppendFormat($"set {columnName} =@isActive");
            sb.Append(" where " + GetColumnName(onlyKey) + " = @Id");
