...
namespace YangOne.IO
{
    ...
    public class FileHelper
    {
        ...
        public static void CopyDirectoryOnly(string source, string target)
        {
            var origin = new DirectoryInfo(source);
            var destination = new DirectoryInfo(target);
            CopyFilesOnly(origin, destination);
        }
        /// <summary>
        ...
        public static void CopyFilesOnly(DirectoryInfo source, DirectoryInfo target, bool overwrite = true)
        {
            foreach (var dir in source.GetDirectories())
            {
                CopyFilesRecursively(dir, target.CreateSubdirectory(dir.Name), overwrite);
