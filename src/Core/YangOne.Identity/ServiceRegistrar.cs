...
namespace YangOne.Identity
{
    public class ServiceRegistrar : IServiceRegistrar
    {
        public void Register(IServiceCollection serviceCollection, IConfiguration configuration)
        {
            ...
            var builder = serviceCollection.AddIdentity<IdentityUser, IdentityRole>(x =>
                {
                    x.SignIn.RequireConfirmedEmail =
                        bool.Parse(configuration["YangOneAppConfig:RequireConfirmedEmail"]);
                    x.Password.RequireDigit = false;
                    x.Password.RequiredLength = int.Parse(configuration["YangOneAppConfig:PasswordLength"]);
                    x.Password.RequireLowercase = false;
                    x.Password.RequireNonAlphanumeric =
                        bool.Parse(configuration["YangOneAppConfig:RequireNonAlphanumeric"]);
                    x.Password.RequireUppercase = bool.Parse(configuration["YangOneAppConfig:RequireUppercase"]);


                }).UseDapperWithSqlServer()
                .AddClaimsPrincipalFactory
                    <YangOne.Identity.ClaimFactory.YangOneClaimsPrincipalFactory<IdentityUser, IdentityRole>>()
                .AddDefaultTokenProviders();

            builder.AddUserClaimsPrincipalFactory();
            //setting up for multiple sub domain identity exchange cookie
            //identity cookie
            serviceCollection.ConfigureApplicationCookie(options =>
            {
                options.Cookie.Name = "_yo_id";
                ...
                options.Events.OnValidatePrincipal = async context =>
                {
                    ...
                    var user = await mgr.UserManager.FindByNameAsync(claimIdentity.Name ??claimIdentity.Claims.FirstOrDefault(x=>x.Type=="name")?.Value);
           ...
           // serviceCollection.AddYangOneIdentitySever(serviceCollection.BuildServiceProvider().GetService<IWebHostEnvironment>(),configuration);
