...
namespace YangOne.Identity.Service
{
    ...
    public class AppUserService : IAppUserService
    {
        ...
        public CrudService<UserType> UserTypeCrudService { get; set; } = new CrudService<UserType>();

        public async Task<UserStatus> SaveNewUserAsync(NewUser model)
        {
        ...
        public async Task<UserStatus> UpdateUserAsync(EditUser model)
        {
            var status = new UserStatus();
            try
            {
        ...
        public async Task<bool> AssignRolesAsync(UserRoles userRoles)
        {
            await _identityUserService.AddUserRoles(userRoles.Roles.Select(e => (int)e.Id).ToArray(), userRoles.IdentityUserId);
            return true;
        }

        public async Task<EditUser> GetAsync(int appuserId)
        {
            var user = await AppUserCrudService.GetAsync(appuserId);
            var userViewModel = user.To<EditUser>();
...
", new { showRecords });
        ...
        public async Task<UserStatus> AddExternalUser(string provider, string providerUserId, List<Claim> claims)
        {
            ...
            foreach (var claim in claims)
            {
                ...
                {
                    filtered.Add(new Claim("name", claim.Value));
                }
            ...
            if (!filtered.Any(x => x.Type == "name"))
            {
                var first = filtered.FirstOrDefault(x => x.Type == "firstname")?.Value;
                var last = filtered.FirstOrDefault(x => x.Type == "surname")?.Value;
                if (first != null && last != null)
                {
                    filtered.Add(new Claim("name", first + " " + last));
                }
                else if (first != null)
                {
                    filtered.Add(new Claim("name", first));
                }
                else if (last != null)
                {
                    filtered.Add(new Claim("name", last));
                }
            }

            // create a new unique subject id
            var sub = Guid.NewGuid().ToString("D");//  CryptoRandom.CreateUniqueId();

            // check if a display name is available, otherwise fallback to subject id
            var name = filtered.FirstOrDefault(c => c.Type == "name")?.Value ?? sub;
            ...
            model.FirstName = filtered.FirstOrDefault(x => x.Type == "given_name")?.Value;
            model.LastName = filtered.FirstOrDefault(x => x.Type == "family_name")?.Value;
            model.Email = filtered.FirstOrDefault(x => x.Type == "email")?.Value;
            model.ProfilePicture = filtered.FirstOrDefault(x => x.Type == "picture")?.Value;
            model.PhoneNumber = filtered.FirstOrDefault(x => x.Type == "phone_number")?.Value;
            model.IsActive = true;
            bool noEmail = string.IsNullOrEmpty(model.Email);
            bool noPhone = string.IsNullOrEmpty(model.PhoneNumber);
            if (noEmail)
            {
                model.Email = model.Password + "@yoframework.com";
        ...
        public async Task<IEnumerable<BasicUserDetails>> GetUserByRole(string search, int RoleId)
        {
            var dbFactory = DbFactoryProvider.GetFactory();
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                await db.OpenAsync();
                return await db.QueryAsync<BasicUserDetails>("[dbo].[usp_AppUser_GetByRoles]",
                    new { RoleId = RoleId, Search = search },
...
where  (au.FirstName like @Query or au.LastName like @Query) and r.Name=@RoleName  Order By au.AddedOn desc OFFSET @Page * (@Page-1) ROWS FETCH NEXT @Limit ROWS ONLY",
        ...
        public async Task<bool> UpdateAsVerifiedPhoneNumberAsync(long userId)
        {
            var dbFactory = DbFactoryProvider.GetFactory();
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                await db.OpenAsync();
                await db.ExecuteAsync("Update  [dbo].[IdentityUser] Set PhoneNumberConfirmed=@PhoneNumberConfirmed where Id=@IdentityUserId ;",
                    new { PhoneNumberConfirmed = true, IdentityUserId = userId });
                return true;
            }
        }
