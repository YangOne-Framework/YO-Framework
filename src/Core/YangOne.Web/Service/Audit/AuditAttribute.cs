    ...
    public class AuditAttribute : Attribute, IAsyncActionFilter
    {
        ...
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            try
            {
                ...
                var config = httpContext.RequestServices.GetService<IOptionsSnapshot<YangOneAppConfig>>();
                if (config.Value.IsInstalled)
                {
            ...
            {
                var logger = ContextResolver.Context.RequestServices.GetService<ILogger>();
