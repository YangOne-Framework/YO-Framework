...
namespace YangOne.Web.Optimizer
{
    public class SmidgeFileProvider : IFileProvider
    {
        ...
        private static readonly char[] InvalidFileNameChars = Path.GetInvalidFileNameChars()
            .Where(c => c != '/' && c != '\\').ToArray();
        public SmidgeFileProvider(IWebHostEnvironment hostingEnvironment)
        {
        ...
        public IDirectoryContents GetDirectoryContents(string subpath)
        {
            ...
            return (IDirectoryContents)new PluginDirectoryContents((IEnumerable<IFileInfo>)fileInfoList);
        }
