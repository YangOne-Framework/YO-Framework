...
namespace YangOne.Web.Grid
{
    ...
    public interface IYOGridColumn
    {
        ...
        IHtmlContent RenderControlSource(object dataSource, IYOGridRow<Object> row);
    ...
    public interface IYOGridColumnsOf<T> : IYOGridColumns<IYOGridColumn<T>>
    {
        ...
        IYOGridColumn<T> Add();
        IYOGridColumn<T> Add<TValue>(Expression<Func<T, TValue>> constraint);
        IYOGridColumn<T> Add<TValue>(Expression<Func<T, TValue>> constraint, string selectedValue = "");

        IYOGridColumn<T> Insert(Int32 index);
        IYOGridColumn<T> Insert<TValue>(Int32 index, Expression<Func<T, TValue>> constraint);
    }
    ...
    public abstract class BaseKachuwaGridColumn<T, TValue> : IYOGridColumn<T>
    {
        ...
        LambdaExpression IYOGridColumn<T>.Expression => Expression;
        ...
        public abstract IHtmlContent RenderControlSource(object dataSource, IYOGridRow<Object> row);
    ...
    public class YOGridColumn<T, TValue> : BaseKachuwaGridColumn<T, TValue> where T : class
    {
        ...
        public YOGridColumn(IYOGrid<T> grid, Expression<Func<T, TValue>> expression)
        {
            Grid = grid;
            IsEncoded = true;
        ...
        public override IHtmlContent RenderControlSource(object dataSource, IYOGridRow<Object> row)
        {
        ...
        private Object GetValueFor(IYOGridRow<Object> row)
        {
            try
            {
                if (RenderValue != null)
    ...
    public class YOGridColumns<T> : List<IYOGridColumn<T>>, IYOGridColumnsOf<T> where T : class
    {
        ...
        public YOGridColumns(IYOGrid<T> grid)
        {
            Grid = grid;
        }

        public virtual IYOGridColumn<T> Add()
        {
            return Add<Object>(model => null);
        }
        public virtual IYOGridColumn<T> Add<TValue>(Expression<Func<T, TValue>> expression)
        {
            IYOGridColumn<T> column = new YOGridColumn<T, TValue>(Grid, expression);
            //Grid.Processors.Add(column);
            Add(column);

            return column;
        }
        public virtual IYOGridColumn<T> Add<TValue>(Expression<Func<T, TValue>> expression, string selectedValue)
        {
            IYOGridColumn<T> column = new YOGridColumn<T, TValue>(Grid, expression);
        ...
        public virtual IYOGridColumn<T> Insert<TValue>(Int32 index, Expression<Func<T, TValue>> expression)
        {
            IYOGridColumn<T> column = new YOGridColumn<T, TValue>(Grid, expression);
            //  Grid.Processors.Add(column);
            Insert(index, column);
