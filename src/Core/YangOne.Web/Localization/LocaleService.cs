...
namespace YangOne.Localization
{
    public class LocaleService : ILocaleService
    {
        ...
        public async Task<IEnumerable<LocaleRegion>> GetRegions()
        {
            ...
            {
                await db.OpenAsync();
                return await db.QueryAsync<LocaleRegion>(@"select l.*,c.NiceName as Country from LocaleRegion l
                inner join Country c on c.CountryId = l.CountryId where l.IsActive = @IsActive and l.IsDeleted = @IsDeleted",
        ...
        public async Task<IEnumerable<LocaleRegionViewModel>> GetLocaleRegions(int page, int limit, string search)
        {
            ...
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                ...
                return await db.QueryAsync<LocaleRegionViewModel>("Select lr.*,c.NiceName as CountryName from " +
                                                            " LocaleRegion as lr inner join Country as c " +
        ...
        public async Task Save(LocaleResource model)
        {
            ...
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                ...
                await db.ExecuteAsync(
                    "if(Not Exists(select 1 from LocaleResource where LTRIM(RTRIM(Name))=LTRIM(RTRIM(@Name)) and LTRIM(RTRIM(Culture))=LTRIM(RTRIM(@Culture)) and " +
        ...
        public async Task<LocaleRegionEditViewModel> GetAllResourcesAsync(int localRegionId, string baseCulture, int page, int limit)
        {
            ...
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                ...
                var resources = await db.QueryAsync<EditLocaleResource>(
                    "SELECT COUNT(1) OVER() AS RowTotal, lr.*,lb.Value as BaseValue from LocaleResource lr left join LocaleResource lb on lb.Name = lr.Name " +
        ...
        public async Task<bool> SetDefaultAsync(int localeRegionId)
        {
            ...
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                await db.OpenAsync();
                await db.ExecuteAsync(
                    "update LocaleResource set IsDefault=@PrevDefault;" +
                    "Update LocaleResource set IsDefault=@IsDefault Where LocaleRegionId=@localeRegionId ",
        ...
        public async  Task<bool> AddNewResourceFromBaseCultureAsync(string culture)
        {
            ...
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                await db.OpenAsync();
                await db.ExecuteAsync(
                    "insert into LocaleResource Select @Culture,Name,'',GroupName from LocaleResource where culture=@BaseCulture;",
        ...
        public async  Task<IEnumerable<LocaleResourcesExportModel>> GetAllResourcesForExportAsync(int localRegionId, string baseCulture)
        {
            ...
            using (var db = (DbConnection)dbFactory.GetConnection())
            {
                ...
                lb.Name, lb.Value, lb.Culture, lb.GroupName from LocaleResource lb
                inner
                    join localeregion as lg on lg.Culture = lb.Culture
                inner
                    join Country as c on c.CountryId = lg.CountryId
                    ...
                    using (var dbTran = db.BeginTransaction())
                    {
                        try
                        {
                            ...
                            foreach (var resource in importedDatas)
                            {
                                ...
                                await db.ExecuteAsync(
                                    @"if(exists(select 1 from  LocaleResource where Culture=@Culture and lower(Name)=lower(@Name)))
                                    begin
