...
namespace YangOne.Web.Form
{
    ...
    public static class YOFormHelpers
    {
        ...
        public static T SetLabelClass<T>(this T input, string className) where T : IFormInput
        {
            input.LabelClass = className;

            return input;
        }
        
        public static T ShowRequiredFlag<T>(this T input) where T : IFormInput
        {
            input.ShowFlag = true;
