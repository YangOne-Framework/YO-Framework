/**
 * YOTheme Studio runtime CSS (blueprint §20) — compiles the resolved theme
 * tokens into CSS variables and the platform-standard .yo-* component classes.
 * Moved out of the legacy YOThemeContext so the studio runtime can consume it
 * without React context.
 */

import { buildFluidScale } from "../pages/Admin/ThemeStudio/themeUtils";
import { normalizeToRuntimeConfig } from "./themeMigration";
 ...
 */
/**
 * mode = "auto" (default): light tokens + @media dark overrides — the real site
 *        follows the OS preference.
 * mode = "dark": light tokens + hard :root dark overrides, never a media query.
 * mode = "light": light tokens only — the OS dark media query is suppressed so
 *        the studio forced-light preview always renders light.
 */
export function buildTokenCss(
  tokens: ParsedThemeConfig["tokens"],
  components?: ParsedThemeConfig["components"],
  ...
  if (darkRules.length > 0) {
    if (mode === "auto") {
      // Real site: follow the OS preference.
      cssOutput += `@media (prefers-color-scheme: dark) {\n:root {\n${darkRules.join("\n")}\n}\n}\n\n`;
    } else if (mode === "dark") {
      // Forced dark preview: override the light :root directly.
      cssOutput += `:root {\n${darkRules.join("\n")}\n}\n\n`;
    }
    // mode === "light" → no dark block (forced light preview always stays light)
  }

  /* ── Responsive breakpoint overrides (min-width media queries) ── */
  const responsiveBlocks: string[] = [];
  for (const [, bp] of Object.entries(tokens.responsive ?? {})) {
    const lines = Object.entries(bp?.vars ?? {}).map(([varName, val]) => `  --${varName}: ${val};`);
    if (lines.length > 0) {
      responsiveBlocks.push(`@media (min-width: ${bp.minWidth}) {\n:root {\n${lines.join("\n")}\n}\n}`);
    }
  }
  if (responsiveBlocks.length > 0) {
    cssOutput += `${responsiveBlocks.join("\n\n")}\n\n`;
  }
...
/**
 * Parse a stored theme config JSON into the flat runtime view.
 * v2 studio configs are normalized back so every consumer renders identically.
 */
export function parseThemeConfig(configJson: string | null | undefined): ParsedThemeConfig | null {
  if (!configJson) return null;
  try {
    return normalizeToRuntimeConfig(JSON.parse(configJson));
  } catch {
    return null;
  }
}
