...
/* ============================================================================
   1.2.0 — Phase 2-6 tables
   ========================================================================== */

/* ── 1. Concurrent editing ── */
IF OBJECT_ID('dbo.YOThemeEditLock', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.YOThemeEditLock (
        EditLockId     BIGINT IDENTITY(1,1) PRIMARY KEY,
        YOThemeId      BIGINT NOT NULL REFERENCES dbo.YOTheme(YOThemeId),
        FieldPath      NVARCHAR(512) NOT NULL,
        LockedBy       BIGINT NOT NULL,
        LockedAt       DATETIME2 NOT NULL DEFAULT GETUTCDATE(),
        Token          NVARCHAR(128) NOT NULL,
        KeptValue      NVARCHAR(MAX) NULL,
        AcceptedValue  NVARCHAR(MAX) NULL
    );
    CREATE INDEX IX_YOThemeEditLock_Theme_Field ON dbo.YOThemeEditLock(YOThemeId, FieldPath);
    CREATE INDEX IX_YOThemeEditLock_Token ON dbo.YOThemeEditLock(Token);
END
GO

/* ── 2. Collaboration — presence & activity ── */
IF OBJECT_ID('dbo.YOThemeCollaboration', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.YOThemeCollaboration (
        CollabId       BIGINT IDENTITY(1,1) PRIMARY KEY,
        YOThemeId      BIGINT NULL REFERENCES dbo.YOTheme(YOThemeId),
        UserId         BIGINT NOT NULL,
        SessionId      NVARCHAR(128) NOT NULL,
        Action         NVARCHAR(64) NOT NULL,        -- joined | left | editing | comment | annotate
        FieldPath      NVARCHAR(512) NULL,
        IPAddress      NVARCHAR(48) NULL,
        StartedOn      DATETIME2 NOT NULL DEFAULT GETUTCDATE(),
        EndedOn        DATETIME2 NULL
    );
    CREATE INDEX IX_YOThemeCollaboration_Theme ON dbo.YOThemeCollaboration(YOThemeId);
    CREATE INDEX IX_YOThemeCollaboration_User ON dbo.YOThemeCollaboration(UserId);
END
GO

/* ── 3. Roles & permissions ── */
IF OBJECT_ID('dbo.YOThemeRole', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.YOThemeRole (
        ThemeRoleId    BIGINT IDENTITY(1,1) PRIMARY KEY,
        ThemeId        BIGINT NULL REFERENCES dbo.YOTheme(YOThemeId),
        UserId         BIGINT NOT NULL,
        Role           NVARCHAR(64) NOT NULL,         -- viewer | editor | designer | developer | brand_reviewer | accessibility_reviewer | publishing_manager | application_manager | platform_admin
        GrantedOn      DATETIME2 NOT NULL DEFAULT GETUTCDATE()
    );
    CREATE INDEX IX_YOThemeRole_Theme ON dbo.YOThemeRole(ThemeId);
    CREATE INDEX IX_YOThemeRole_User ON dbo.YOThemeRole(UserId);
END
GO

IF OBJECT_ID('dbo.YOThemePermission', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.YOThemePermission (
        PermissionId   BIGINT IDENTITY(1,1) PRIMARY KEY,
        Role           NVARCHAR(64) NOT NULL,
        Action         NVARCHAR(64) NOT NULL,         -- preview | edit_simple | edit_tokens | edit_css | manage_assignments | publish
        Allowed        BIT NOT NULL DEFAULT 1
    );
    INSERT INTO dbo.YOThemePermission (Role, Action, Allowed) VALUES
        ('viewer',              'preview',                1),
        ('viewer',              'edit_simple',            0), ('viewer',              'edit_tokens',            0), ('viewer',              'edit_css',               0), ('viewer',              'manage_assignments',     0), ('viewer',              'publish',                0),
        ('editor',              'preview',                1), ('editor',              'edit_simple',            1), ('editor',              'edit_tokens',            1), ('editor',              'edit_css',               0), ('editor',              'manage_assignments',     0), ('editor',              'publish',                0),
        ('designer',            'preview',                1), ('designer',            'edit_simple',            1), ('designer',            'edit_tokens',            1), ('designer',            'edit_css',               0), ('designer',            'manage_assignments',     0), ('designer',            'publish',                0),
        ('developer',           'preview',                1), ('developer',           'edit_simple',            1), ('developer',           'edit_tokens',            1), ('developer',           'edit_css',               1), ('developer',           'manage_assignments',     0), ('developer',           'publish',                0),
        ('publishing_manager',  'preview',                1), ('publishing_manager',  'edit_simple',            1), ('publishing_manager',  'edit_tokens',            1), ('publishing_manager',  'edit_css',               1), ('publishing_manager',  'manage_assignments',     1), ('publishing_manager',  'publish',                1),
        ('platform_admin',      'preview',                1), ('platform_admin',      'edit_simple',            1), ('platform_admin',      'edit_tokens',            1), ('platform_admin',      'edit_css',               1), ('platform_admin',      'manage_assignments',     1), ('platform_admin',      'publish',                1);
END
GO

/* ── 4. Figma live sync webhook ── */
IF OBJECT_ID('dbo.YOThemeFigmaWebhook', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.YOThemeFigmaWebhook (
        FigmaWebhookId  BIGINT IDENTITY(1,1) PRIMARY KEY,
        IntegrationId   BIGINT NOT NULL REFERENCES dbo.YOThemeIntegration(ThemeIntegrationId),
        FigmaFileKey    NVARCHAR(128) NOT NULL,
        WebhookUrl      NVARCHAR(512) NOT NULL,
        Secret          NVARCHAR(256) NULL,
        LastReceivedOn  DATETIME2 NULL,
        IsActive        BIT NOT NULL DEFAULT 1,
...
/* ── 5. CVD simulation results cache ── */
IF OBJECT_ID('dbo.YOThemeCVDResult', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.YOThemeCVDResult (
        CVDResultId    BIGINT IDENTITY(1,1) PRIMARY KEY,
        YOThemeId      BIGINT NOT NULL REFERENCES dbo.YOTheme(YOThemeId),
        SimulationType NVARCHAR(64) NOT NULL,          -- protanopia | deuteranopia | tritanopia | achromatopsia | protanomaly | deuteranomaly | tritanomaly | achromatomaly
        IndistinguishablePairs NVARCHAR(MAX) NULL,    -- JSON array of pairs
        ChartCollisions        NVARCHAR(MAX) NULL,    -- JSON array
        StatusOnlyHueIssues    NVARCHAR(MAX) NULL,    -- JSON array
        CalculatedOn   DATETIME2 NOT NULL DEFAULT GETUTCDATE()
    );
    CREATE INDEX IX_YOThemeCVDResult_Theme ON dbo.YOThemeCVDResult(YOThemeId);
END
GO

/* ── 6. Token dependency graph cache ── */
IF OBJECT_ID('dbo.YOThemeTokenGraph', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.YOThemeTokenGraph (
        TokenGraphId   BIGINT IDENTITY(1,1) PRIMARY KEY,
        YOThemeId      BIGINT NOT NULL REFERENCES dbo.YOTheme(YOThemeId),
        TokenPath      NVARCHAR(512) NOT NULL,
        DependsOn      NVARCHAR(512) NULL,
        UsedBy         NVARCHAR(MAX) NULL,           -- JSON array of token paths
        UsageCount     INT NOT NULL DEFAULT 0,
        ComponentUsages NVARCHAR(MAX) NULL,          -- JSON array
        PageUsages     NVARCHAR(MAX) NULL,          -- JSON array
        CalculatedOn   DATETIME2 NOT NULL DEFAULT GETUTCDATE()
    );
    CREATE INDEX IX_YOThemeTokenGraph_Theme ON dbo.YOThemeTokenGraph(YOThemeId);
    CREATE INDEX IX_YOThemeTokenGraph_Token ON dbo.YOThemeTokenGraph(TokenPath);
END
GO

/* ── 7. Performance budget ── */
IF OBJECT_ID('dbo.YOThemePerformanceBudget', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.YOThemePerformanceBudget (
        BudgetId       BIGINT IDENTITY(1,1) PRIMARY KEY,
        YOThemeId      BIGINT NOT NULL REFERENCES dbo.YOTheme(YOThemeId),
        Metric         NVARCHAR(64) NOT NULL,        -- cssSize | criticalCssSize | variableCount | componentRuleCount | duplicateRuleCount | fontCount | fontFileSize | imageSize | compileTime | themeResolutionTime | previewUpdateTime
        Value          FLOAT NOT NULL,
        Unit           NVARCHAR(32) NULL,
        Threshold      FLOAT NULL,
        Passed         BIT NULL,
        CheckedOn      DATETIME2 NOT NULL DEFAULT GETUTCDATE()
    );
    CREATE INDEX IX_YOThemePerformanceBudget_Theme ON dbo.YOThemePerformanceBudget(YOThemeId);
END
GO

/* ── 8. Bulk edit queue ── */
IF OBJECT_ID('dbo.YOThemeBulkEdit', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.YOThemeBulkEdit (
        BulkEditId     BIGINT IDENTITY(1,1) PRIMARY KEY,
        YOThemeId      BIGINT NOT NULL REFERENCES dbo.YOTheme(YOThemeId),
        Action         NVARCHAR(64) NOT NULL,         -- bulkReplace | bulkRename | bulkModeMapping | bulkLock | bulkUnlock | bulkDeleteUnused
        SearchPattern  NVARCHAR(512) NULL,
        ReplacePattern NVARCHAR(512) NULL,
        DryRun         BIT NOT NULL DEFAULT 1,
        AffectedCount  INT NULL,
        Status         NVARCHAR(32) NOT NULL DEFAULT 'pending',  -- pending | running | completed | failed
        CreatedBy      BIGINT NOT NULL,
        CreatedOn      DATETIME2 NOT NULL DEFAULT GETUTCDATE(),
        CompletedOn    DATETIME2 NULL
    );
    CREATE INDEX IX_YOThemeBulkEdit_Theme ON dbo.YOThemeBulkEdit(YOThemeId);
END
GO

/* ── 10. Theme analytics aggregation ── */
IF OBJECT_ID('dbo.YOThemeAnalyticsSummary', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.YOThemeAnalyticsSummary (
        SummaryId      BIGINT IDENTITY(1,1) PRIMARY KEY,
        YOThemeId      BIGINT NULL REFERENCES dbo.YOTheme(YOThemeId),
        DateKey        INT NOT NULL,                  -- YYYYMMDD
        Metric         NVARCHAR(64) NOT NULL,         -- usage | assignment_count | light_dark_usage | component_variant_usage | unused_tokens | most_overridden | accessibility_trends | publish_frequency | schedule_performance | experiment_results | css_performance_trends
        Value          FLOAT NOT NULL,
        Dimension      NVARCHAR(128) NULL
    );
    CREATE INDEX IX_YOThemeAnalyticsSummary_Theme_Date ON dbo.YOThemeAnalyticsSummary(YOThemeId, DateKey);
    CREATE INDEX IX_YOThemeAnalyticsSummary_Metric ON dbo.YOThemeAnalyticsSummary(Metric);
END
GO

/* ── 11. Display preferences ── */
IF OBJECT_ID('dbo.YOThemeDisplayPreference', 'U') IS NULL
BEGIN
    CREATE TABLE dbo.YOThemeDisplayPreference (
        DisplayPrefId  BIGINT IDENTITY(1,1) PRIMARY KEY,
        YOThemeId      BIGINT NOT NULL REFERENCES dbo.YOTheme(YOThemeId),
        PreferenceKey  NVARCHAR(64) NOT NULL,         -- highContrast | reducedMotion | largeText | compact | comfortable
        PreferenceValue NVARCHAR(32) NOT NULL,
        AppliedOn      DATETIME2 NOT NULL DEFAULT GETUTCDATE()
    );
    CREATE INDEX IX_YOThemeDisplayPreference_Theme ON dbo.YOThemeDisplayPreference(YOThemeId);
END
GO
