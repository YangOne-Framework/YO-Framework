...
BEGIN
    ...
    BEGIN
        INSERT INTO dbo.YOTheme (YOThemeUniqueId, Name, Slug, [Version], Author, Config, Description, IsActive, AddedBy, UpdatedBy)
        VALUES (@YOThemeUniqueId, @Name, @Slug, @Version, @Author, @Config, @Description, @IsActive, @UpdatedBy, @UpdatedBy);
        SELECT SCOPE_IDENTITY() AS YOThemeId, 'inserted' AS Action;
    END
    ELSE
    BEGIN
        UPDATE dbo.YOTheme SET Name = @Name, Slug = @Slug, [Version] = @Version, Author = @Author, Config = @Config, Description = @Description, UpdatedOn = GETDATE(), UpdatedBy = @UpdatedBy
