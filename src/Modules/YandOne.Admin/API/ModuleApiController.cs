...
public class ModuleApiController : BaseApiController
{
    ...
    [HttpPost("install")]
    [Authorize(Roles = "Admin,SuperAdmin")]
    public async Task<ActionResult<ApiResponse<bool>>> Install(ModuleActionRequest request)
    {
        ...
        try
        {
            ...
            if (string.IsNullOrWhiteSpace(request.ModuleName))
                return ErrorResponse<bool>(400, "Invalid module.");

            var module = await _moduleManager.FindAsync(request.ModuleName);
            if (module == null)
                return ErrorResponse<bool>(404, "Module not found.");

            var ok = await _moduleManager.InstallAsync(module);

            if (!ok)
                return ErrorResponse<bool>(500, "Module installation failed.");
    ...
    [HttpPost("uninstall")]
    [Authorize(Roles = "Admin,SuperAdmin")]
    public async Task<ActionResult<ApiResponse<bool>>> Uninstall(ModuleActionRequest request)
    {
        if (!ModelState.IsValid)
            return ErrorResponse<bool>(ModelState, 600, request);
        ...
        try
        {
            ...
            if (!ok)
                return ErrorResponse<bool>(500, "Module uninstallation failed.");
